...
namespace SCS.Api.Services
{
    ...
    public class FsoService : IFsoService
    {
        ...
        public FsoDTO ToDTO(FileSystemObject fso)
        {
            return new FsoDTO(fso);
        }

        public List<FsoDTO> ToDTO(List<FileSystemObject> list)
        {
            var result = new List<FsoDTO>();
            foreach (var f in list)
            {
                result.Add(new FsoDTO(f));
            }
            return result;
        }
        ...
        {
            var userDirectory = Path.Combine(_storageUrl, user.Id);
        ...
        public async Task<Stream> GetFileAsync(FileSystemObject root, List<FileSystemObject> fsoList, User user)
        {
            ...
            {
                var fileFullPath = Path.Combine(_storageUrl, user.Id, fsoList[0].FileName);
        ...
        private async Task AddFsoToArchiveAsync(ZipArchive archive, FileSystemObject fso, FileSystemObject root, User user)
        {
            ...
            if (!fso.IsFolder)
            {
                var fullPath = Path.Combine(_storageUrl, user.Id, fso.FileName);
